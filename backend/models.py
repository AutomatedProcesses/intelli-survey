...
class Response(db.Model):
  ...
  responder_answers = db.relationship('ResponderAnswer', cascade='all,delete', back_polulates='response')
...
class Survey(db.Model):
  ...
  obj_id = db.Column(db.Integer, primary_key=True)
  hash_key = db.Column(db.Integer, unique=True, nullable=False)
  name = db.Column(db.String(100), unique=True, nullable=False)
  description = db.Column(db.Text, nullable=True)
  date_created = db.Column(db.DateTime, default=lambda: datetime.now(timezone.utc))
  
  questions = db.relationship('Question', cascade='all,delete', back_polulates='survey')
